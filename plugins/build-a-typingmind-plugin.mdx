...
sidebarTitle: "Build Custom Plugins"
---
...
<Info>
  All existing TypingMind plugins are open-source under MIT license. You can view the source code of the plugins from our public [GitHub repository](https://github.com/TypingMind) here. The repos that contain the plugins are ones with names that start with `plugin-`. You can fork or create your own plugin from these plugins.
</Info>
...
**2. Read the latest rendered output from the plugin**

All plugins can read their own latest output whose type is not `respond_to_ai`. This includes `render_html`, `render_markdown`, `cards`, and legacy HTML/Markdown output types. There is no permission required for this resource.

- Permission ID: (No permission needed)
- Resource variable name: `previousRunOutput`
- Example data: the output value from the most recent qualifying run, or `undefined` if there is no qualifying output.

<Note>
  TypingMind searches backward through the current chat for the latest qualifying output from the same plugin, including its other functions. Calls with output type `respond_to_ai` are skipped. This resource returns one output, not the entire chat history or another plugin's output.
</Note>

<Warning>
  **Breaking change:** Previously, `previousRunOutput` returned the immediate previous run's output, including `respond_to_ai` results. It now skips `respond_to_ai` results, so it can return an older rendered output or `undefined`. Plugins that use AI-only results to pass state between calls must pass that state explicitly in function parameters or store it separately.
</Warning>

For example, after a plugin renders HTML, its Read Source or Search Source functions can return `respond_to_ai` results without replacing the HTML available to the next read or update call.
...
When hosting your plugin on GitHub, you can set `codeFile` on a JavaScript entry in `pluginFunctions` to keep its implementation in a separate `.js` file. TypingMind loads that file during GitHub import. See [JavaScript source files](/plugins/typingmind-plugin-json-schema#javascript-source-files) for the format and an example.
...
| `OAUTH_USER_ID_TOKEN` | The `id_token` of the user. Only available if the current user is authenticated using [OAuth](/typingmind-team/user-management/o-auth-2-0). |
| `OAUTH_USER_ACCESS_TOKEN` | The `access_token` of the user. Only available if the current user is authenticated using [OAuth](/typingmind-team/user-management/o-auth-2-0). |
