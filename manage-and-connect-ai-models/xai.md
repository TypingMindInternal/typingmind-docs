...
## Step 5: Use xAI on TypingMind

- Go to [**typingmind.com**](http://typingmind.com)
- Navigate the Models menu on the left sidebar → Switch to API key tab
- Input the copied API key for xAI.

<Frame>
  ![Api Key Tab](/images/api-key-tab.webp)
</Frame>

After entering the API key for xAI, switch back to Models tab → select xAI → enable xAI models you want to chat with.

<Frame>
  ![Xai Model](/images/xai-model.webp)
</Frame>
